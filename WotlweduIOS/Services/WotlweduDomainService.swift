...
final class WotlweduDomainService {
    ...
    // MARK: - AI
    func electionRecommendations(electionId: String) async throws -> AIElectionRecommendations {
        let endpoint = Endpoint(path: "ai/election/\(electionId)/recommendations", method: .get)
        let response: APIResponse<AIElectionRecommendations> = try await api.send(endpoint)
        guard let data = response.data else {
            throw APIError.server(message: response.message ?? "No recommendations", url: nil)
        }
        return data
    }

    func suggestListItems(prompt: String, count: Int = 5) async throws -> AIListSuggestions {
        struct Payload: Encodable { let prompt: String; let count: Int }
        let endpoint = Endpoint(
            path: "ai/list/suggest-items",
            method: .post,
            body: try JSONEncoder.api.encode(Payload(prompt: prompt, count: count))
        )
        let response: APIResponse<AIListSuggestions> = try await api.send(endpoint)
        guard let data = response.data else {
            throw APIError.server(message: response.message ?? "No suggestions", url: nil)
        }
        return data
    }

    func electionSummary(electionId: String) async throws -> AIElectionSummary {
        let endpoint = Endpoint(path: "ai/election/\(electionId)/summary", method: .get)
        let response: APIResponse<AIElectionSummary> = try await api.send(endpoint)
        guard let data = response.data else {
            throw APIError.server(message: response.message ?? "No summary", url: nil)
        }
        return data
    }

    func notificationDigest() async throws -> AINotificationDigest {
        let endpoint = Endpoint(path: "ai/notification/digest", method: .get)
        let response: APIResponse<AINotificationDigest> = try await api.send(endpoint)
        guard let data = response.data else {
            throw APIError.server(message: response.message ?? "No digest", url: nil)
        }
        return data
    }

    func suggestParticipants(electionId: String, limit: Int = 5) async throws -> AIParticipantSuggestions {
        let endpoint = Endpoint(
            path: "ai/election/\(electionId)/suggest-participants",
            method: .get,
            query: [URLQueryItem(name: "limit", value: "\(limit)")]
        )
        let response: APIResponse<AIParticipantSuggestions> = try await api.send(endpoint)
        guard let data = response.data else {
            throw APIError.server(message: response.message ?? "No participant suggestions", url: nil)
        }
        return data
    }

    func categorizeText(_ text: String) async throws -> AICategoryResult {
        struct Payload: Encodable { let text: String }
        let endpoint = Endpoint(
            path: "ai/item/categorize",
            method: .post,
            body: try JSONEncoder.api.encode(Payload(text: text))
        )
        let response: APIResponse<AICategoryResult> = try await api.send(endpoint)
        guard let data = response.data else {
            throw APIError.server(message: response.message ?? "No category result", url: nil)
        }
        return data
    }

    func moderateText(_ text: String) async throws -> AIModerationResult {
        struct Payload: Encodable { let text: String }
        let endpoint = Endpoint(
            path: "ai/moderate",
            method: .post,
        ...
        let response: APIResponse<AIModerationResult> = try await api.send(endpoint)
        guard let data = response.data else {
            throw APIError.server(message: response.message ?? "No moderation result", url: nil)
        }
        return data
    }

    func describeImage(imageId: String) async throws -> AIImageDescription {
        let endpoint = Endpoint(path: "ai/image/\(imageId)/describe", method: .get)
        let response: APIResponse<AIImageDescription> = try await api.send(endpoint)
        guard let data = response.data else {
            throw APIError.server(message: response.message ?? "No image description", url: nil)
        }
        return data
    }

    func smartDefaults() async throws -> AISmartDefaults {
        let endpoint = Endpoint(path: "ai/preferences/defaults", method: .get)
        let response: APIResponse<AISmartDefaults> = try await api.send(endpoint)
        guard let data = response.data else {
            throw APIError.server(message: response.message ?? "No smart defaults", url: nil)
        }
        return data
    }

    func assistantQuery(_ query: String) async throws -> AIAssistantResponse {
        struct Payload: Encodable { let query: String }
        let endpoint = Endpoint(
            path: "ai/assistant/query",
            method: .post,
            body: try JSONEncoder.api.encode(Payload(query: query))
        )
        let response: APIResponse<AIAssistantResponse> = try await api.send(endpoint)
        guard let data = response.data else {
            throw APIError.server(message: response.message ?? "No assistant response", url: nil)
        }
        return data
    }
